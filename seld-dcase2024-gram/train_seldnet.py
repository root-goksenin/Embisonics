...
def get_accdoa_labels(accdoa_in, nb_classes):
    x, y, z = accdoa_in[:, :, :nb_classes], accdoa_in[:, :, nb_classes:2*nb_classes], accdoa_in[:, :, 2*nb_classes:3*nb_classes]
    dist = accdoa_in[:, :, 3*nb_classes:]
    sed = np.sqrt(x**2 + y**2 + z**2) > 0.5
    return sed, accdoa_in, dist
    ...
    with torch.no_grad():
        for values in data_generator.generate():
            ...
            if params['multi_accdoa'] is True:
                ...
                sed_pred, doa_pred, dist_pred = get_accdoa_labels(output.detach().cpu().numpy(), params['unique_classes'])
                ...
                for frame_cnt in range(sed_pred.shape[0]):
                    for class_cnt in range(sed_pred.shape[1]):
                        if sed_pred[frame_cnt][class_cnt]>0.5:
                            if frame_cnt not in output_dict:
                                output_dict[frame_cnt] = []
                            output_dict[frame_cnt].append([class_cnt, doa_pred[frame_cnt][class_cnt], doa_pred[frame_cnt][class_cnt+params['unique_classes']], doa_pred[frame_cnt][class_cnt+2*params['unique_classes']], dist_pred[frame_cnt][class_cnt]])
    ...
    with torch.no_grad():
        for values in data_generator.generate():
            ...
            if params['multi_accdoa'] is True:
                mags = []
                for tr in range(3):
                    base = tr * 4 * nb_classes  # x, y, z, dist per track
            ...
            if params['multi_accdoa'] is True:
                sed_pred0, doa_pred0, dist_pred0, sed_pred1, doa_pred1, dist_pred1, sed_pred2, doa_pred2, dist_pred2 = get_multi_accdoa_labels(out_np, params['unique_classes'])
                sed_pred0 = reshape_3Dto2D(sed_pred0)
                doa_pred0 = reshape_3Dto2D(doa_pred0)
                dist_pred0 = reshape_3Dto2D(dist_pred0)
                sed_pred1 = reshape_3Dto2D(sed_pred1)
                ...
                sed_pred, doa_pred, dist_pred = get_accdoa_labels(out_np, params['unique_classes'])
                sed_pred = reshape_3Dto2D(sed_pred)
                doa_pred = reshape_3Dto2D(doa_pred)
                dist_pred = reshape_3Dto2D(dist_pred)
...
def main(argv):
        ...
        for split_cnt, split in enumerate(test_splits):
            ...
            if params['modality'] == 'audio_visual':
                ...
                sphere = SphereV4(patch_strategy=PatchStrategy(fshape=16, fstride=16,
                                ...
                                in_channels=7)
                sphere.load_state_dict(torch.load("/gpfs/work4/0/prjs1338/spherev3/InChannels=7/Fraction=None/UseMSEIV=NoneModel=GRAM-T/ModelSize=base/LR=0.0004/BatchSize=64/NrSamples=None/Patching=frame/InputL=500/step=8000.ckpt")['state_dict'], strict=True)
                sphere.to(device)
                mono = seldnet_model.gramt_mono_spec(sphere.gram, n_freq=sphere.gramt_n_freq,
                                    embed_dim=sphere.gramt_native_dim)
                mono.model = mono.model.to(device)

                model = seldnet_model.SphereV4SELD(data_out, 
                                    params, 
                                    mono, 
                                    sphere, 
            ...
            print('SED metrics: F-score: {:0.1f} {}'.format(100* test_F[0]  if use_jackknife else 100* test_F, '[{:0.2f}, {:0.2f}]'.format(100* test_F[1][0], 100* test_F[1][1]) if use_jackknife else ''), flush=True)
            print('DOA metrics: Angular error: {:0.1f} {}'.format(test_LE[0] if use_jackknife else test_LE, '[{:0.2f} , {:0.2f}]'.format(test_LE[1][0], test_LE[1][1]) if use_jackknife else ''), flush=True)
            print('Distance metrics: {:0.2f} {}'.format(test_dist_err[0] if use_jackknife else test_dist_err, '[{:0.2f} , {:0.2f}]'.format(test_dist_err[1][0], test_dist_err[1][1]) if use_jackknife else ''), flush=True)
            print('Relative Distance metrics: {:0.2f} {}'.format(test_rel_dist_err[0] if use_jackknife else test_rel_dist_err, '[{:0.2f} , {:0.2f}]'.format(test_rel_dist_err[1][0], test_rel_dist_err[1][1]) if use_jackknife else ''), flush=True)

            if params['average']=='macro':
                print('Classwise results on unseen test data', flush=True)
                print('Class\tF\tAE\tdist_err\treldist_err\tSELD_score', flush=True)
                for cls_cnt in range(params['unique_classes']):
                    print('{}\t{:0.2f} {}\t{:0.2f} {}\t{:0.2f} {}\t{:0.2f} {}\t{:0.2f} {}'.format(
                        ...
                        '[{:0.2f}, {:0.2f}]'.format(classwise_test_scr[1][6][cls_cnt][0],
                                                    classwise_test_scr[1][6][cls_cnt][1]) if use_jackknife else ''), flush=True)
    ...
    if params['mode'] == 'eval':
        ...
        cls_feature_class.delete_and_create_folder(dcase_output_test_folder)
        print('Dumping recording-wise eval results in: {}'.format(dcase_output_test_folder), flush=True)
