...
class WXYZDecoder(nn.Module):
    """Reconstructs W + X/Y/Z log-mels of the clean (unrotated) scene.
    Lightweight auxiliary head that provides rich inter-channel acoustic physics
    (phase/polarity, reverberation, distance cues) to complement the primary
    IV directionality objective.
    """
...
class IVDecoder(nn.Module):
    ...
    Primary directionality head — remains the focus of the model.
    """
    def __init__(self, encoder_embed_dim: int, decoder_embed_dim: int,
                 depth: int, num_heads: int, patch_shape: tuple,
                 num_patches: int, grid_size: tuple, mlp_ratio: float = 2.0):
...
# ======================================================================
# SphereV2 — Directionality-first model (WXYZ auxiliary + IV primary)
# ======================================================================

class SphereV2(pl.LightningModule):
    """Directionality-first architecture:
      - Pure self-attention spatial encoder on 7-channel input (WXYZ log-mels + 3 IV).
      - Two reconstruction heads (NO semantic/GRAMT cross-attention):
          • WXYZDecoder: clean (unrotated) W + X/Y/Z log-mels → rich spatial physics
          • IVDecoder (FiLM-R): rotated IV field + geometric consistency loss
      - Primary objective = strong, rotation-equivariant directionality that
        complements pre-trained mono semantic models (e.g. GRAMT-mono).
      - WXYZ is lightweight auxiliary supervision only.
    """
    ...
    def __init__(
            ...
            wxyz_loss_weight: float = 0.25,   # modest weight so IV dominates
        ...
        # --- Spatial encoder (pure self-attention, no semantic branch) ---
    ...
    def forward(self, x_clean, x_rot_flat, R_mats_flat, visible_mask):
        ...
        # iv_target, w_target = self._iv_target(iv_target_clean)
        ...
        # Total Loss
        total = self.wxyz_loss_weight * l_wxyz + l_consistency + l_iv_rot

        # total = self.wxyz_loss_weight * l_wxyz
