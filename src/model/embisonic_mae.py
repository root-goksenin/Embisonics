...
class EmbisonicsMAE(pl.LightningModule):
    ...
    def forward(self, x_clean, x_rot_flat, R_mats_flat, visible_mask):
        ...
        pred_clean = self.decoder(z_vis, visible_mask, R_id_6d)               # (N, P, 3, fs, ts)
        ...
        R_6d_flat = matrix_to_6d(R_mats_flat)
        pred_rot = self.decoder(z_vis_rep, visible_mask_rep, R_6d_flat)       # (N*Rn, P, 3, fs, ts)
        ...
        total = l_rot_masked + l_clean_masked
    ...
    def generate_clip_level_steering_plot(self, x_clean, num_steps=36, save_path="steering_sweep.png"):
        ...
        for theta in angles_rad:
            ...
            pred_iv = self.decoder(z_vis, full_mask, R_6d) # (1, P, 3, fs, ts)
    ...
    def generate_masked_steering_plot(self, x_clean, mask_ratio=0.75, save_path="steering_masked.png"):
        ...
        for ang in angles_deg:
            ...
            pred_iv = self.decoder(z_vis, visible_mask, R_6d) # (1, P, 3, fs, ts)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.global_step % self.diag_every_n_steps == 0 and self.global_step > 0:
            with torch.no_grad():
                ...
                pred_noR = self.decoder(z_vis_rep, vmask_rep, torch.zeros_like(out["R_6d_flat"]))
