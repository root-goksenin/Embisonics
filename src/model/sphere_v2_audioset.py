...
class WXYZDecoder(nn.Module):
    """Reconstructs W + X/Y/Z log-mels of the clean (unrotated) scene. No R
    conditioning — this decoder is purely a reconstruction head on the encoder
    output.
    """
    def __init__(self, encoder_embed_dim: int, decoder_embed_dim: int,
        ...
        # 4 channels: W + X/Y/Z
        self.head = nn.Linear(decoder_embed_dim, 4 * patch_shape[0] * patch_shape[1])
    ...
    def forward(self, z_vis, visible_mask):
        ...
        for i, blk in enumerate(self.blocks):
            x = blk(x)
        x = self.norm(x)
        return self.head(x).view(N, P, 4, self.patch_shape[0], self.patch_shape[1])


class IVDecoder(nn.Module):
    """Reconstructs unit IV direction field. FiLM-R conditioned. Dedicated
    trunk — no task interference with log-mel reconstruction.
    """
    def __init__(self, encoder_embed_dim: int, decoder_embed_dim: int,
                 depth: int, num_heads: int, patch_shape: tuple,
...
class SphereV2(pl.LightningModule):
    """v2 architecture:
      ...
      - GRAMT-mono (frozen) processes UNMASKED W log-mel, cross-attended at each
        spatial encoder layer. GRAMT 'raw' is unpacked + reordered to (f,t) and
        receives the same patch-portion PE as the spatial encoder.
      - Two decoders consuming ONLY encoder output:
          WXYZ predictor:  W + X/Y/Z log-mel of the CLEAN scene (no R)
          SpatialDecoder:  masked & non-masked IVs of ROTATED scene + Identity Scene (FiLM-R)
    """
    ...
    def _init_our_weights(self):
        ...
        for mod in [self.patch_embed, self.gramt_proj,
                    ...
                    self.wxyz_decoder, self.iv_decoder]:
        ...
        # FiLM zero-init lives only on the IV decoder now.
    ...
    def _gramt_tokens(self, w_log_mel_full: torch.Tensor) -> torch.Tensor:
        ...
        assert FD == F_gram * D, f"Expected last dim {F_gram*D}, got {FD}"
        assert T == self.gramt_t_dim, f"Expected T_gram={self.gramt_t_dim}, got {T}"
    ...
    def generate_clip_level_steering_plot(self, x_clean, num_steps=36,
        ...
        # Fully-visible encoder pass (GRAMT + encoder handled inside).
    ...
    def _prepare_batch(self, batch):
        audio, noise, noise_length, noise_idx, source_rir, noise_rirs, snr, context_idx, R_mats = batch
        # 1. Generate scene
        audio = generate_scenes_batch.generate_scene(
    ...
    def forward(self, x_clean, x_rot_flat, R_mats_flat, visible_mask):
        ...
        # GRAMT context + encoder on clean visible patches.
        w_full = x_clean[:, 0:1]
        gramt_tokens = self._gramt_tokens(w_full)
        z = self._encode_visible(x_clean, visible_mask, gramt_tokens)
        z_vis = z[:, 1:, :]

        # Targets
        wxyz_target_clean = self._patchify(x_clean, slice(0, 4))
        ...
        # Predictions
        wxyz_pred = self.wxyz_decoder(z_vis, visible_mask)
        ...
        l_wxyz = masked_mse_loss(wxyz_pred, wxyz_target_clean, masked)

        masked = ~visible_mask
        masked_rep = ~visible_mask_rep

        # WXYZ is still masked MSE
        l_wxyz = masked_mse_loss(wxyz_pred, wxyz_target_clean, masked)

        # Total IV Rot Loss (applies to the entire reproduced IV field)
        ...
        # Mathematically rotate the clean predictions
        iv_pred_geom_rot = geometric_rotate_iv(iv_pred_clean_rep, R_mats_flat).detach()

        # CONSISTENCY LOSS: Applied to the full field as well
        l_consistency = cosine_iv_loss(iv_pred_rot, iv_pred_geom_rot, w_iv_rot, self.loss_sharpness)

        # Total Loss
        total = 0.25 * l_wxyz + l_iv_rot + l_consistency

        return {
            "loss": total,
            "l_wxyz": l_wxyz,
            "l_iv_rot": l_iv_rot,              # <-- Replaced the two separate ones
            "l_consistency": l_consistency,
            "wxyz_pred": wxyz_pred, "wxyz_target": wxyz_target_clean,
            "iv_pred": iv_pred_rot, "iv_target": iv_unit_rot,
            "iv_weight_target": w_iv_rot,
            "masked": masked, "masked_rep": masked_rep,
    ...
    def _log_reconstruction(self, pred_patches: torch.Tensor,
        ...
        pred_patches, target_patches: (N, P, C, fs, ts)
        """
        with torch.no_grad():
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.global_step % self.log_every_n_steps == 0:
            # Inputs: clean and rotated (first sample, all 7 channels in plot_fbank).
            ...
            # W/X/Y/Z prediction vs clean X/Y/Z target (first sample).
        ...
        if self.global_step % self.diag_every_n_steps == 0 and self.global_step > 0:
            with torch.no_grad():
                ...
                print(f"--- step {self.global_step} | "
                    ...
                    f"l_wxyz={out['l_wxyz']:.4f}  ca_norm={ca_norm:.4f}")
