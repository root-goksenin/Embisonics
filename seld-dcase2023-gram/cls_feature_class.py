...
def resample(audio, orig_sr, target_sr):
    return torchaudio.functional.resample(
            audio, orig_sr, target_sr, **RESAMPLE_KWARGS
        )
...
class SpearFeatureExtractor:
    """SPEAR front-end = raw 16 kHz W-channel waveform (no learned extractor).

    SPEAR consumes the waveform directly, so we cache the resampled W-channel
    laid out on the feature-frame grid: the samples for each feat frame become
    one row -> on-disk shape (1, samples_per_frame, T).  The data generator
    rides these rows through the same circular buffer as Dasheng, then flattens
    (T, spf) back into a contiguous waveform per sequence for the frozen encoder.
    """
    sr = 16000
    def __init__(self, hop_len_s):
        self.samples_per_frame = int(round(hop_len_s * self.sr))   # 0.02 s -> 320

    def __call__(self, audio_resampled, nb_feat_frames):
        wav = audio_resampled                                      # (1, N) @ 16 kHz
        if isinstance(wav, torch.Tensor):
            wav = wav.detach().cpu().numpy()
        wav = np.asarray(wav, dtype=np.float32)
        if wav.ndim == 2:
            wav = wav[0]                                           # (N,)
        target = nb_feat_frames * self.samples_per_frame
        if wav.shape[0] >= target:
            wav = wav[:target]
        else:
            wav = np.pad(wav, (0, target - wav.shape[0]), mode='constant')
        wav = wav.reshape(nb_feat_frames, self.samples_per_frame)  # (T, spf)
        return wav.T[np.newaxis, ...]                              # (1, spf, T)
...
class FeatureClass:
    def __init__(self, params, is_eval=False):
        ...
        self.spear_extract = SpearFeatureExtractor(self._hop_len_s)
    ...
    # add to FeatureClass
    def get_spear_feat_dir(self):
            return os.path.join(self._feat_label_dir, '{}_spear'.format(self._dataset_combination))
    ...
    def extract_file_feature(self, _arg_in):
                ...
                if self._dataset == 'foa':
                    ...
                    audio = audio[[0]] #Take the W channel
                    audio_resampled = resample(audio=audio,
                                               orig_sr = sr, 
                                               target_sr = self.spear_extract.sr)
                    spear_feat = self.spear_extract(audio_resampled, feat.shape[0]) 
                ...
                if feat is not None:
                    ...
                    print('{}: {}, {}'.format(_file_cnt, os.path.basename(_wav_path), spear_feat.shape ))

                    np.save(_feat_path, feat)
                    _spear_dir = self.get_spear_feat_dir(); create_folder(_spear_dir)

                    np.save(os.path.join(_spear_dir, os.path.basename(_feat_path)), spear_feat)
