...
#
# Dasheng integration: when params['mono_encoder'] starts with 'dasheng', the
# data generator yields (feat, dash, label) and the model is the two-stream
# DashengSphereSELD called as model(feat, dash). For GRAM the original
...
def _is_dasheng(params):
    return str(params.get('mono_encoder', '')).startswith('dasheng')
...
def test_epoch(data_generator, model, criterion, dcase_output_folder, params, device):
    ...
    use_dasheng = _is_dasheng(params)
...
def train_epoch(data_generator, optimizer, model, criterion, params, device):
    ...
    for batch in data_generator.generate():
        ...
        output = model(data, dash) if use_dasheng else model(data)
...
def main(argv):
    ...
    for split_cnt, split in enumerate(test_splits):
        ...
        #   - dasheng-*: external mono stream is frozen Dasheng on cached mel,
        #                model is called as model(feat, dash).
        #   - gram     : original GRAMT-mono path, model is called model(feat).
        # ---------------------------------------------------------------
        if _is_dasheng(params):
            dasheng_id = params['mono_ckpts'][params['mono_encoder'].replace('-', '_')]
            mono = embisonics.DashengMono(dasheng_id).to(device)
            mono_spec = embisonics.dasheng_mono_spec(mono)
            model = embisonics.DashengSphereSELD(
