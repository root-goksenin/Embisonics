...
class DashengFeatureExtractor:
    ...
    sr = 16000
    def __init__(self):
        self.feature_extractor = AutoFeatureExtractor.from_pretrained(self.model_name, trust_remote_code=True)
    def __call__(self, audio):
        if audio.ndim == 1:
            audio = audio.unsqueeze(0)
        assert audio.ndim == 2, f"Audio shape is {audio.shape}"
        features = self.feature_extractor(audio, sampling_rate=self.sr, return_tensors="pt")["input_values"]
        assert features.ndim == 3
        assert features.shape[1] == 64
        assert features.shape[2] == (audio.shape[-1] // (self.sr // 100)) + 1, f"audio shape is {audio.shape}, features: {features.shape} "
...
class FeatureClass:
    def __init__(self, params, is_eval=False):
        ...
        self.dasheng_extract = DashengFeatureExtractor()
    ...
    # add to FeatureClass
    def get_dasheng_feat_dir(self):
        return os.path.join(self._feat_label_dir, '{}_dasheng'.format(self._dataset_combination))
    ...
    def extract_file_feature(self, _arg_in):
                ...
                if self._dataset == 'foa':
                    ...
                    audio_resampled = resample(audio=audio,
                                               orig_sr = sr, 
                                               target_sr = self.dasheng_extract.sr)
                    dasheng_feat = self.dasheng_extract(audio_resampled)
                ...
                if feat is not None:
                    ...
                    print('{}: {}, {}'.format(_file_cnt, os.path.basename(_wav_path), dasheng_feat.shape))
                    np.save(_feat_path, feat)
                    _dash_dir = self.get_dasheng_feat_dir(); create_folder(_dash_dir)
                    np.save(os.path.join(_dash_dir, os.path.basename(_feat_path)), dasheng_feat)
